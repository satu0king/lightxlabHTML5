...
function setup() {
    ...
    radialSourceList = [];
    lightBeamList=[];
    // lightBeamList.push(new lightBeam(100,100,200,0));
    // m.push(new mirror(100, 100,  200, 0 * Math.PI / 180));
    // m.push(new mirror(100, 200,  200, 30 * Math.PI / 180));
    l = [];
    for (var i = 0; i < 360; i += 5) {
        //l.push(new lightRay(0, 0, i * Math.PI / 180));
    }
    // rs = new radialSource(100, 100);
    uiArea.setup();
...
var uiArea = {
    ...
    setup: function() {
        ...
        window.addEventListener('mousedown', function(e) {
            // // // // console.log(uiArea.mouseX+":"+uiArea.mouseY);
            var rect = uiArea.canvas.getBoundingClientRect();
...
var simulationArea = {
    canvas: document.getElementById("simulationArea"), //   .createElement("canvas"),
    reset: false,
    setup: function() {
    ...
    clear: function() {
        this.context.clearRect(0, 0, this.canvas.width, this.canvas.height);
        // // // console.log("simulationArea:cleared");
    }
}

function update() {
    // uiArea.selected=false;
    ...
    // simulationArea.sourceChanged |= rs.update();
        ...
        for (var i = 0; i < 1; i++) {
            ...
            for (var i = 0; i < mirrorList.length; i++) {
                ...
                if (soln != null) {
                    ...
                    var dis = distance(this.x, this.y, soln["x"], soln["y"]);
                    if ((soln["y"] - this.y) * Math.sin(this.angle) > 0 && dis > 0.1 && dis < closestDistance) {
                        xx = soln["x"];
                        yy = soln["y"];
                        newAngle = soln["angle"];
                        closestDistance = dis;
...
function lightBeam(x, y, width, angle) {
    ...
    this.angle = angle;
    this.color = "green";
    this.leftButton = new Button(x, y, 7);
    this.rightButton = new Button(x + width * Math.cos(angle), y + width * Math.sin(angle), 7);
